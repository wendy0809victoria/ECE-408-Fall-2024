...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (row < dim) {
    float dot = 0;
    int num_elem = matRows[row]; 
    for (int i = 0; i < num_elem; i++) {  
      int elem = matColStart[i];  
      if (matCols[row+elem] < dim) {  
        dot += matData[row+elem] * vec[matCols[row+elem]];
      }
    }
    if (matRowPerm[row] < dim) {
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  int BLOCK_SIZE = dim;
  dim3 gridDim(ceil(dim/BLOCK_SIZE), 1, 1);  
